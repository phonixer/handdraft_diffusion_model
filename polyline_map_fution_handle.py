...
from diffusion_model_fution import Diffusion
...
# 生成 x 张量，并转换为浮点类型
trajs_future = torch.tensor(obj_trajs_future[track_index_to_predict,::4,:2]).to(device)
# 生成 state 张量，每个 batch 的值都相同，并转换为浮点类型
trajs_past = torch.tensor(obj_trajs_past[track_index_to_predict,:,:2]).to(device)

# x 的每个点都减去第一个点
trajs_future = (trajs_future - trajs_future[:, 0:1, :]) / 50
trajs_past = (trajs_past - trajs_future[:, 0:1, :]) / 50
# x[0,:,:] = x[3,:,:]
# state[0,:,:] = state[3,:,:]
trajs_future[0,:,:] = trajs_future[3,:,:]
trajs_past[0,:,:] = trajs_past[3,:,:]
trajs_future[2,:,:] = trajs_future[4,:,:]
trajs_past[2,:,:] = trajs_past[4,:,:]


print('trajs_future:', trajs_future.shape)
print('trajs_past:', trajs_past.shape)
...
train_model = False
# 计算trajs_future的尺寸
# num_points_each_polylines = trajs_future.shape[1]   

mlp_out_dim = num_polylines * num_points_each_polylines * in_channels + trajs_past.shape[1] * 2
act_dim = num_polylines * num_points_each_polylines * in_channels + trajs_future.shape[1] * 2
...
x = x.to(device)
polylines_mask = polylines_mask.unsqueeze(-1).expand_as(x).contiguous()
x = x.view(batch_size, -1)
print('x.shape', x.shape)
trajs_future = trajs_future.view(batch_size, -1)
trajs_past = trajs_past.view(batch_size, -1)
x_in = torch.cat([x, trajs_past], dim=1)
x_out = torch.cat([x, trajs_future], dim=1)
...
# 对mask进行扩展 多的都是1
polylines_mask = polylines_mask.view(batch_size, -1)
trajs_future_mask = torch.ones_like(trajs_future).view(batch_size, -1).to(device)
print('polylines_mask.shape', polylines_mask.shape)
print('trajs_future_mask.shape', trajs_future_mask.shape)
mask = torch.cat([polylines_mask, trajs_future_mask], dim=1)


print('mask.shape', mask.shape)

state = {'polylines': x_in, 'polylines_mask': mask}
...
# Split x_out into map and trajectory parts
x_out_map = x_out[:, :num_polylines * num_points_each_polyline * in_channels]
x_out_traj = x_out[:, num_polylines * num_points_each_polyline * in_channels:]

# Reshape map and trajectory parts
x_out_map = x_out_map.view(batch_size, num_polylines, num_points_each_polyline, in_channels)
x_out_traj = x_out_traj.view(batch_size, -1, in_channels)

# Restore shapes for plotting
x_out_map = x_out_map.cpu().detach().numpy()
x_out_traj = x_out_traj.cpu().detach().numpy()
...
axes = axes.flatten()
# trajs_past = trajs_past.view(batch_size, -1, 2).cpu().detach().numpy()
...
for i in range(num_center_objects):
    ...
    for j in range(num_of_src_polylines):
        ...
        ax.plot(points_action[:, 0], points_action[:, 1], 'r', label='Map Predicted' if j == 0 else "")
        ax.plot(points_x_test[:, 0], points_x_test[:, 1], 'g', label='Map Ground Truth' if j == 0 else "")
    # ax.plot(center_objects[i, 0], center_objects[i, 1], 'ro', label='Center')
    # ax.plot(trajs_past[i, :, 0], trajs_past[i, :, 0], 'bo', label='Initial Position')
    ax.plot(x_out_traj[i, :, 0], x_out_traj[i, :, 1], 'b', label='Predicted Trajectory')
...
for step_idx in range(T):
    step = diffusion_steps[step_idx][:, :num_polylines * num_points_each_polyline * in_channels].view(batch_size, num_polylines, num_points_each_polylines, in_channels)
    # 逆归一化
...
for i in range(len(polt_item)):
    ...
    for j in range(len(steps_to_plot)):
        step = diffusion_steps[steps_to_plot[j]][:, :num_polylines * num_points_each_polyline * in_channels].view(batch_size, num_polylines, num_points_each_polyline, in_channels)
